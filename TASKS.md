...
45. **Majestic offline software staging complete:** Firmware `work/fh8626v100-majestic@4bd2f8bc...` now carries the ABI probe, sensor 0x7c->0x68 facade, source VMM and DSP/SYS/VPSS/VENC facades, fixed native 720p25 H.264/VBR bring-up, stream descriptor translation with balanced leases, strict/permissive/native-video runners, and a backend `/metrics` proxy that preserves the stock frontend. Builder now composes the Majestic target from `work/fh8626v100-anjia@0945bd3...`; the former separate Majestic Builder branch is archived.
...
51. **Next gate is build/runtime/hardware, not more speculative platform reverse.** Build the composed target, verify default HTTP/WebUI plus live `/metrics`, run `majestic-fh8626-abi-probe`, then exercise strict -> permissive-stub -> native-video in that order. Native-video acceptance is VI -> VENC -> balanced stream leases -> sustained RTSP. Only after that evidence exists should the actually-observed Majestic ISP/JPEG/audio call surfaces be wrapped. Do not fabricate unobserved FH8852 structures.
