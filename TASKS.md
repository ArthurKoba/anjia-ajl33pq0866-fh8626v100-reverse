...
51. **Next gate is build/runtime/hardware, not more speculative platform reverse.** Build the composed target, verify the untouched Majestic HTTP/WebUI baseline, record that `/metrics` is still empty or otherwise characterize the real provider behavior, run `majestic-fh8626-abi-probe`, then exercise strict -> permissive-stub -> native-video in that order. Native-video acceptance is VI -> VENC -> balanced stream leases -> sustained RTSP. Exercise the new RTX audio facade only after the actual Majestic audio call path is observed. Fix `/metrics` at the Majestic/platform provider boundary; do not add a second web server or patch frontend JavaScript.
