...
45. **Majestic build staging advanced:** Firmware `work/fh8626v100-majestic@7fd1ee93...` inherits the streamer-neutral core, keeps the isolated FH8852V200 control-plane compatibility package, adds the non-mutating ABI probe, and now carries the first source sensor-ABI facade (FH8852 0x7c -> FH8626 GC1054 0x68). Builder now composes the Majestic target from `work/fh8626v100-anjia@0945bd3...`; the former separate Majestic Builder branch is archived.
...
51. Next source gate: statically close the exact FH8852 SYS/VPSS/VENC function signatures and record layouts required by Majestic before replacing donor `libdsp` calls. Reuse literal ioctl overlap where proven; do not replace working/common requests gratuitously. The next hardware run should first exercise the new GC1054 sensor facade, then proceed to VI -> VENC -> sustained RTSP before ISP tuning. After base video is stable, validate ISP/color/exposure/day-night, audio capture/playback/two-way behavior, restart, and board PTZ/illumination integration.
