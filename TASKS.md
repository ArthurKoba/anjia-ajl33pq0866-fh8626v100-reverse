...
31. Keep factory `.ko/.so/.bin` out of the clean Firmware contribution. All 15 unique opaque payloads are an explicit reverse/recovery backlog in `docs/process/fh8626-blob-retirement.md`: every kernel module, userspace plug-in, sensor/profile object and ARC firmware contract must be reconstructed. SDK provenance may provide a legitimate implementation source, but an opaque SDK copy alone does not close the task.
