...
Current staging is split correctly: Firmware `work/fh8626v100-majestic@7fd1ee93...` owns the generic compatibility package; Builder now uses the single composed-device line `work/fh8626v100-anjia@0945bd3...`, whose `fh8626v100_lite_anjia-ajl33pq0866_majestic` fragment selects that Firmware direction. The former separate Builder Majestic branch is retired and retained only as an archive tag. No FH8852 kernel modules or firmware are installed. See `docs/process/fh8626-majestic-staging.md`.

The Firmware staging package contains the non-mutating `majestic-fh8626-abi-probe` plus the first media-facing source adapter. Static comparison proved that FH8626 GC1054 returns a 0x68-byte sensor callback table while multiple FH8852V200 plug-ins use a differently ordered 0x7c-byte table. The branch now builds an FH8852-shaped GC1054 facade instead of relying on direct callback-table compatibility. The default service remains media-off. This is source staging, not target acceptance.
