...
Current staging is split correctly: Firmware `work/fh8626v100-majestic@e00cae3d...` owns the generic compatibility package; Builder now uses the single composed-device line `work/fh8626v100-anjia@0945bd3...`, whose `fh8626v100_lite_anjia-ajl33pq0866_majestic` fragment selects that Firmware direction. The former separate Builder Majestic branch is retired and retained only as an archive tag. No FH8852 kernel modules or firmware are installed. See `docs/process/fh8626-majestic-staging.md`.

The Firmware staging package has reached the offline media-adapter boundary. It contains the non-mutating ABI probe; an FH8852-shaped GC1054 facade over the recovered FH8626 0x68-byte sensor contract; source `libvmm.so` and `libdsp.so` facades for the recovered VMM/SYS/VPSS/VENC surface; a fixed native 1280x720@25 H.264 Baseline/VBR bring-up path; FH8626 stream acquire/release translated into the FH8852 public stream representation with balanced leases; loader-complete optional DSP/JPEG symbols as explicit staging stubs; and a source `libacw_mpi.so` facade translating the recovered FH8852 audio MPI subset onto the hardware-proven FH8626 `/dev/rtxbus` transport. The normal service remains media-off.
...
This is `SOFTWARE_MEDIA_STAGING / BUILD_AND_HARDWARE_PENDING`, not completed product support. Arbitrary VENC/RC layouts, full ISP replacement, Majestic JPEG behavior, full audio/two-way policy, day/night integration and same-boot opaque ISP teardown remain runtime-evidence-dependent. The moving donor `master` binary must still be pinned/reproduced before product acceptance.
