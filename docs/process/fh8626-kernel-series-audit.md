...
The clean Firmware candidate no longer carries an AJL fragment at all. The named ANJIA Builder profile is the owner of this selection and must use `CONFIG_FH8626V100_SD0_1BIT=y` when rebuilt on the curated Linux source. The preserved Firmware/Builder snapshots keep the old symbol only as history.
